...
class TestParsePlanResponse:
    ...
    def test_parse_multiple_jsons_only_first(self):
        """测试解析多个 JSON 时只使用第一个"""
        raw = """{"type": "tap", "thought": "第一个", "log": "test", "target": "按钮1"}
{"type": "tap", "thought": "第二个", "log": "test", "target": "按钮2"}"""
        result = parse_plan_response(raw)
        assert isinstance(result, TapPlanAction)
        assert result.target == "按钮1"  # 应该使用第一个 JSON

    def test_parse_json_in_markdown_block(self):
        """测试解析 markdown 代码块中的 JSON"""
        raw = '```json\n{"type": "tap", "thought": "test", "log": "test", "target": "按钮"}\n```'
        result = parse_plan_response(raw)
        assert isinstance(result, TapPlanAction)
    ...
    def test_parse_nested_json_objects(self):
        """测试解析包含嵌套对象的 JSON"""
        raw = '{"type": "done", "thought": "完成", "log": "完成", "return_result": true, "result": "包含{}嵌套的内容"}'
        result = parse_plan_response(raw)
        assert isinstance(result, DonePlanAction)
        assert result.result == "包含{}嵌套的内容"

    def test_parse_json_with_string_containing_braces(self):
        """测试解析字符串中包含括号的 JSON"""
        raw = r'{"type": "done", "thought": "测试", "log": "测试", "result": "包含 {括号} 的内容"}'
        result = parse_plan_response(raw)
        assert isinstance(result, DonePlanAction)
        assert result.result is not None and "括号" in result.result

    def test_parse_invalid_json_raises_error(self):
        """测试解析无效 JSON 时抛出错误"""
        raw = '{"type": "tap", "target": "按钮"'  # 缺少闭合括号
    ...
    def test_parse_multiple_jsons_with_different_types(self):
        """测试解析多个不同类型的 JSON"""
        raw = """{"type": "swipe", "thought": "滑动", "log": "滑动", "direction": "up"}
{"type": "tap", "thought": "点击", "log": "点击", "target": "按钮"}"""
        result = parse_plan_response(raw)
        assert isinstance(result, SwipePlanAction)
        assert result.direction == "up"  # 应该使用第一个（swipe）

    def test_parse_json_with_escapes_in_strings(self):
        """测试解析包含转义字符的 JSON"""
        raw = r'{"type": "input", "thought": "测试", "log": "测试", "text": "包含\"引号\"的内容"}'
        result = parse_plan_response(raw)
        assert isinstance(result, InputPlanAction)
        assert "引号" in result.text
